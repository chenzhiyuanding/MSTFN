...
def select_device(device='', batch_size=0, newline=True):
    ...
    s = f'MSTFN 🚀 {git_describe() or file_update_date()} torch {torch.__version__} '  # string
...
def model_info(model, verbose=False, img_size=320):
    ...
    try:  # FLOPs
        ...
        img = torch.zeros((1, model.yaml.get('ch', 3),3, stride, stride), device=next(model.parameters()).device)  # input
        flops = profile(deepcopy(model), inputs=(img,), verbose=False)[0] / 1E9 * 2  # stride GFLOPs
        lkw1,lkw2 = profile(deepcopy(model), inputs=(img,), verbose=False)  # stride GFLOPs
